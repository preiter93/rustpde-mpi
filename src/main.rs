...
fn mainxx() {
    ...
    navier.init_random(1e-3);
    // navier.callback();
    let (funval, grad_adj) = navier.grad_adjoint(10., Some(1.), 0.5, 0.5, None);
    println!("Function Value: {:?}", funval);
...
fn mainxxy() {
    ...
    // let (funval, grad_adj) = navier.grad_adjoint(10., Some(1.), 0.5, 0.5, None);
    // println!("Function Value: {:?}", funval);
    integrate(&mut navier, 30., Some(1.0));
}
...
fn main() {
    ...
    let ti_list = ndarray::Array::linspace(5., 50., 10);
    let en_list = ndarray::Array::logspace(10., 0., 3., 14);
